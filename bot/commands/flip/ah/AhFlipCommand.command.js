...
class AhFlipCommand extends Command{
    constructor(prefix,msg){
        super(prefix,msg);

        if(this.command == "flip" && this.args[0] == "ah" || this.command == "flip" && this.args[0] == "auction"){
            this.AhFlip();
        }
    }
    ...
    async AhFlip(){
        ...
        if(!isNaN(parseInt(this.args[1]))){
            ...
            var res = await this.http.post("http://localhost:8080/api/v1/nitric/auction/advise/profit/rt",{"profit":parseInt(amount)}).catch(e =>{
                finalMsg.edit("***The main server has crashed, please try again in a few seconds (if this continues to happen join the support server to see the current status of the bot by clicking the link on my about me)***");
                error = true;
            });
            if(error == true || !res){
        ...
        await this.msg.reply("**Do `!flip ah [amount]` instead**");

        return;
    }
}

module.exports = AhFlipCommand;